package com.provider.service7001.controller;

import com.netflix.hystrix.contrib.javanica.annotation.HystrixCommand;
import com.provider.service7001.entity.User;
...
public class UserController {
    ...
    public User findUsers(@PathVariable("id") Long id){
        System.out.println("===============================/user===================================");
        if(-1 == id){
            throw new RuntimeException("当前数据返回异常");
        }
        User user = new User(11L, "rbb",25);
        return user;
    }

    public User userFallback(@PathVariable("id") Long id){
        return new User(-1L, "服务端提示：error:当前id："+ id +"@HystrixCommand(fallbackMethod = \"userFallback\")",-1);
    }
